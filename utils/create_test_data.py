...
def main(ct_scans_folder, effusions_folder, output_folder, save_scan_probability):
    ...
    for ct_scan_file in os.listdir(ct_scans_folder):
        if ct_scan_file.endswith(".nii"):
            ...
            ct_data = ct_scan_img.get_fdata()[:, :, :]
            effusion_data = effusion_img.get_fdata()[:, :, :]

            if np.random.rand() <= save_scan_probability:
                file_extension = ".npy.h5" 
                ...
                with h5py.File(output_path, "w") as hf:
                    hf.create_dataset("image", data=ct_data)
                    hf.create_dataset("label", data=effusion_data)

    # Perform any additional operations as needed
    print("Processing complete.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process CT scans and effusion masks.")
    ...
    parser.add_argument("--save_scan_probability", type=float, default=0.3, help="Probability of saving each scan.")

    args = parser.parse_args()
    main(args.ct_scans_folder, args.effusions_folder, args.output_folder, args.save_scan_probability)
